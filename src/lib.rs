...
use protobuf::{descriptor, compiler_plugin};
use protoc::{Protoc, DescriptorSetOutArgs};
    ...
    P: AsRef<Path>
{
    paths.iter().map(|input| {
        match input.as_ref().to_str() {
            ...
            None => Err(format_err!("failed to convert {:?} to string", input.as_ref()))
        }
    }).collect()
}

fn write_out_generated_files<P>(generation_results: Vec<compiler_plugin::GenResult>, output_dir: P)
    -> CompileResult<()>
where
    P: AsRef<Path>
{
...
pub fn compile_grpc_protos<Input, Include, Output>(inputs: &[Input], includes: &[Include], output: Output)
    -> CompileResult<()>
where
    Input: AsRef<Path>,
    Include: AsRef<Path>,
    Output: AsRef<Path>
{
    ...
    protoc.check().context("failed to find `protoc`, `protoc` must be availabe in `PATH`")?;
    ...
    protoc.write_descriptor_set(DescriptorSetOutArgs {
        out: match descriptor_set.as_ref().to_str() {
            Some(s) => s,
            None => bail!("failed to convert descriptor set path to string")
        },
        input: stringified_inputs.iter().map(String::as_str).collect::<Vec<&str>>().as_slice(),
        includes: stringified_includes.iter().map(String::as_str).collect::<Vec<&str>>().as_slice(),
    ...
    let descriptor_set = protobuf::parse_from_bytes::<descriptor::FileDescriptorSet>(&serialized_descriptor_set)
        .context("failed to parse descriptor set")?;

    let files_to_generate = inputs.iter().map(|input| {
        match input.as_ref().file_name().map(OsString::from) {
            Some(i) => Ok(i.into_string().map_err(|e| format_err!("failed to convert {:?} to string", e))?),
            None => Err(format_err!("failed to find file name for {:?}", input.as_ref()))
        }
